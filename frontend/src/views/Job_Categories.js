...
const createPolarAreaChartData = (job) => {
  ...
  const salaryRanges = labels.map((label) => {
    const minSalary = Math.min(
      ...job.results
        .filter((result) => result.category.label === label)
        .map((result) => result.salary_min)
    );
    const maxSalary = Math.max(
      ...job.results
        .filter((result) => result.category.label === label)
        .map((result) => result.salary_max)
    );
...
const JobCategories = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const responseHistory = await fetch(
      "/cached_responses/salary_history_uk.json",
      {
        method: "GET",
      }
    )
    ...
    const responseSearch = await fetch(
      "/cached_responses/search_results.json",
      {
        method: "GET",
      }
    )
      ...
      <section className="map">
        <div className="grid grid-cols-2 grid-rows-2 gap-10">
          <div>
            ...
            {searchData && (
              <Scatter data={createScatterChartData(searchData)} />
            )}
          </div>
          <div>
          ...
          <div>
            <h2>Average Salary in each Category</h2>
            {searchData && (
              <PolarArea data={createPolarAreaChartData(searchData)} />
            )}
          </div>
        </div>
      </section>
