...
export const getBlogs = async () => {
    const response = await fetch('https://ip2-backend.vercel.app/api/blog/')
    const data = await response.json()
    return data
}

export const getBlog = async (id) => {
    const response = await fetch(`https://ip2-backend.vercel.app/api/blog/${id}/`)
    const data = await response.json()
    return data
}

export const addContact = async (name, email, message) => {
    const response = await fetch(`https://ip2-backend.vercel.app/api/contact/add/`, {
        method: "POST",
        headers: {
